## ASCII Lab

ASCII Lab is a **FIGlet-style ASCII art generator** with a modern, terminal-inspired UI, live preview, and flexible export options (TXT, PNG, transparent PNG, and QR links).

### Features

- **FIGlet fonts**: Work with a curated set of popular FIGlet fonts, with keyboard navigation and search.
- **Hash-based state**: Shareable URLs with all settings encoded in the hash (text, font, width, layouts, preview size).
- **Layout controls**: Horizontal/vertical layout modes, automatic or fixed width, and adjustable preview font size.
- **Exports**:
  - Copy raw ASCII to the clipboard.
  - Download `.txt` files.
  - Export PNG or transparent PNG.
  - Generate QR codes that link to a simplified output page (`ascii.html`).
- **Mobile-friendly**: Adaptive controls and bottom action bar for smaller screens.

### Tech stack

- **Frontend**: React 19 + TypeScript + Vite 6
- **Styling**: Tailwind CSS (via `@tailwindcss/vite`) with custom CSS
- **ASCII rendering**: `figlet`
- **UI utilities**: `lucide-react`, `motion`, `clsx`, `tailwind-merge`

### Getting started

#### Prerequisites

- Node.js 20+ (recommended)
- npm (comes with Node)

#### Installation

```bash
npm install
```

#### Development

```bash
npm run dev
```

This starts Vite on port `3000` (hosted on `0.0.0.0`), with the app mounted at `/`.

#### Linting / type checking

```bash
npm run lint
```

Runs TypeScript in `--noEmit` mode to ensure the project type-checks cleanly.

#### Build

```bash
npm run build
```

Outputs a production build into the `dist` directory.

#### Preview

```bash
npm run preview
```

Serves the production build locally for final verification.

#### Clean

```bash
npm run clean
```

Removes the `dist` directory in a cross‑platform way.

### Environment variables

Environment variables are defined via `.env` files. A sample file is provided as `.env.example`.

- `GEMINI_API_KEY`: API key for Gemini if you extend the app with AI-powered features.
- `APP_URL`: Public URL where the app is hosted (useful if you add callbacks or self-referential links).

Create your own `.env` (not committed to git) based on `.env.example` if you need these values.

### Deployment

This repository is set up to deploy with **GitHub Pages**:

- The workflow in `.github/workflows/deploy-pages.yml`:
  - Builds the app on pushes to `main`.
  - Uploads the `dist` folder as a Pages artifact.
  - Deploys to GitHub Pages using `actions/deploy-pages`.

Once Pages is configured in your repository settings, pushes to `main` will automatically publish the latest build.

### Project structure (high level)

- `src/App.tsx` – Main application UI and logic.
- `src/main.tsx` – React entry point.
- `src/index.css` – Tailwind and global styles.
- `public/ascii.html` – Minimal output-only page used for QR and link sharing.
- `fonts/` – Local FIGlet font folder (optional, documented in `fonts/readme.txt`).

### Contributing

Issues and pull requests are welcome. When contributing, please:

- Run `npm run lint` before opening a PR.
- Keep the UI consistent with the existing visual language (terminal / cyber aesthetic).

